...
//  get() {
//    val constantPool = Class::class.java.getDeclaredMethod("getConstantPool").apply { isAccessible = true }.invoke(javaClass) as sun.reflect.ConstantPool
//    println("Constant Pool Size = ${constantPool.size}")
//    for (i in constantPool.size downTo 0) {
//      try {
//        val memberRefInfo = constantPool.getMemberRefInfoAt(i)
//        println(memberRefInfo)
//        memberRefInfo.forEach { println(it) }
//
//        val name = memberRefInfo[1]
...
//  }
  get() {
    val theUnsafe = Util.getTheUnsafe()

    val jvmVersion = System.getProperty("java.specification.version", "0").toDouble();
    val constantPoolName = if (jvmVersion < 9) "sun.reflect.ConstantPool" else "jdk.internal.reflect.ConstantPool"

    val constantPoolClass = Class.forName(constantPoolName)
    val getConstantPool = Class::class.java.getDeclaredMethod("getConstantPool")
    val getSize = constantPoolClass.getDeclaredMethod("getSize")
    val getMemberRefInfoAt = constantPoolClass.getDeclaredMethod("getMemberRefInfoAt", Int::class.java)

    val overrideField = AccessibleObject::class.java.getDeclaredField("override")
    val overrideFieldOffset = theUnsafe.objectFieldOffset(overrideField)

    theUnsafe.putBoolean(getConstantPool, overrideFieldOffset, true)
    theUnsafe.putBoolean(getSize, overrideFieldOffset, true)
    theUnsafe.putBoolean(getMemberRefInfoAt, overrideFieldOffset, true)

    val constantPool = getConstantPool.invoke(Object::class.java)
    val constantPoolSize = getSize.invoke(constantPool) as Int

    println("Constant Pool Size = $constantPoolSize")

    for (i in constantPoolSize downTo 0) {
      try {
        val memberRefInfo = getMemberRefInfoAt.invoke(constantPool, i) as? Array<*>
        //memberRefInfo?.forEach { println(it) }

        val name = memberRefInfo?.get(1) as? String
        println("Name = $name")

        // Autogenerated ($), constructor, or kotlin's check (fix maybe?)
        if (name?.contains("(\\$|<init>|checkParameterIsNotNull)".toRegex()) == true) {
          continue
        } else {
          return name
        }
